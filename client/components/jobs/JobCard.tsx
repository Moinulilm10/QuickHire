...
export default function JobCard({ job, index }: JobCardProps) {
  return (
    <Link href={`/jobs/${job.uuid}`} className="block group">
      <div
        className="bg-white border border-surface-border p-6 hover:shadow-card transition-all duration-[var(--transition-base)] cursor-pointer animate-fade-in-up h-full"
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-between items-start">
            <div
              ...
              style={{
                backgroundColor: job.logoColor || "var(--brand-primary)",
              }}
            >
              {job.logoUrl ? (
                <Image
